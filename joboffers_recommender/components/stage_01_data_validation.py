...
class DataValidation:
    ...
    def preprocess_data(self):
        try:
            ...
            joboffers = pd.read_csv(self.data_validation_config.joboffers_csv_file, sep=";", error_bad_lines=False, encoding='latin-1')
            
            logging.info(f" Shape of ratings data file: {ratings.shape}")
            logging.info(f" Shape of joboffers data file: {joboffers.shape}")

            #Here Image URL columns is important for the poster. So, we will keep it
            joboffers = joboffers[['ISBN','Book-Title', 'Book-Author', 'Year-Of-Publication', 'Publisher','Image-URL-L']]
            # Lets remane some wierd columns name in joboffers
            joboffers.rename(columns={"Book-Title":'title',
                                'Book-Author':'author',
            ...
            # Lets store users who had at least rated more than 200 joboffers
            x = ratings['user_id'].value_counts() > 200
            y = x[x].index
            ratings = ratings[ratings['user_id'].isin(y)]

            # Now join ratings with joboffers
            ratings_with_joboffers = ratings.merge(joboffers, on='ISBN')
            number_rating = ratings_with_joboffers.groupby('title')['rating'].count().reset_index()
            number_rating.rename(columns={'rating':'num_of_rating'},inplace=True)
            final_rating = ratings_with_joboffers.merge(number_rating, on='title')

            # Lets take those joboffers which got at least 50 rating of user
